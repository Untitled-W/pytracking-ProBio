...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    if report_name == 'yt2019_vos':
        return extract_results_vos(trackers, dataset, report_name, skip_missing_seq=skip_missing_seq,
                                     plot_bin_gap=plot_bin_gap, exclude_invalid_frames=exclude_invalid_frames,verbose=verbose)
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        # Load anno
        # print(type(seq), seq, seq.ground_truth_rect)
...
def extract_results_vos(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
                    exclude_invalid_frames=False, verbose=True):
