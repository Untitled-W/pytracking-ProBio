...
import argparse
parser = argparse.ArgumentParser(description='TrackNet')
...
tkl = [Tracker(args.tracker_name, args.tracker_param)]
...
#     result = one_track_single(tkl[i][-1])
#     trk,param = tkl[i][0], tkl[i][1]
#     result.to_csv(os.path.join('../tracking_csv', 'result_{}_{}.csv'.format(trk, param)), index=False)
result = one_track(tkl[0])
result.to_csv(os.path.join('../tracking_csv', 'result_{}_{}.csv'.format(args.tracker_name, args.tracker_param)), index=False)